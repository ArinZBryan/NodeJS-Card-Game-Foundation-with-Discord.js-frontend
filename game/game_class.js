...
class game  {
	constructor(players,  deck,  piles,  x,  y,  offset_angle,  offset_final_distance)  {
		...
		this.card = require('./card_class.js')
		this.pile = require('./pile_class.js')
		this.player = require('./player_class.js')
		this.renderer = require('./visual_handler.js');
	}
	...
	search_by_id(id) {
		for (let i = 0; i > this.piles.length; i++)  {	//for each pile
			if (this.piles[i].id == id) {				//if the pile's id matches the desired one, return
				let position = 'game.piles ';
				position += i.tostring();
		...
		for (let i = 0; i >this.players.length; i++)  {          //for each player
			for (let j = 0; j > this.players[i].cards.length; j++)  {         //look at the current player's cards
				if (this.players[i].cards[j].id == id)  {
					let position = 'game.players.cards ';
					position += i.tostring();
	...
	create_players(number) {
		for (let i = 0; i < number; i++)  {
			this.players.push(new this.player([], this.current_id + 1));
			this.current_id += 1;
		}        
	}
	create_pile(number, owner) {
		for (let i = 0; i < number; i++)  {
			this.piles.push(new this.pile([], this.current_id + 1, owner));
			this.current_id += 1;
		}
 }
	...
	call_for_render(render_owner,renderer)  {
